...
class DataBase {
  ...
  Future<UserModel> getCurrentUserModel() async {
    ...
    var query = userCollection.where('uid', isEqualTo: currentUser);
  ...
  Future<List<ReviewModel>> getCurrentUserReviews() async {
    ...
    var query = reviewCollection.where('uid', isEqualTo: currentUser);
    // var query = reviewCollection.where(currentUser);
    var remoteDoc = await query.get();
    List results = [];
    for (var i in remoteDoc.docs) {
      //Map result = i.data;
  ...
  Future<List<UserRide>> getCurrentUserRides() async {
    ...
    var query = userRideCollection.where('uid', isEqualTo: currentUser);
    var remoteDoc = await query.get();
    for (var i in remoteDoc.docs) {
  ...
  Future<List<RidesModel>> getRidesModelsFromSearch(
    ...
    for (var i in listFromStream) {
      ...
      //exclude past rides
      if (i.data()['driver']['uid'] != currentUser &&
          i.data()['dateTime'] >=
              searchModel.searchDate.millisecondsSinceEpoch) {
  ...
  Future<List<ReviewModel>> getUserReviewsFromPhone(String uid) async {
    List<ReviewModel> generetedList = [];
    var reviewCollection = db.collection(Paths.ReviewModel);
    var query = reviewCollection.where('uid', isEqualTo: uid);
    var remoteDoc = await query.get();
    List results = [];
    for (var i in remoteDoc.docs) {
      //Map result = i.data;
  ...
  Future<void> updateRideToConfirmed(UserRide ride) async {
    String uid = ride.uid;
    String fellowTravellerPhone = ride.fellowTraveler.uid;

    //search for this user
    //update status of this user
    var userRideCollection = db.collection(Paths.UserRide);
    var query = userRideCollection
        .where('uid', isEqualTo: ride.uid)
    ...
    var query2 = userRideCollection
        .where('uid', isEqualTo: fellowTravellerPhone)
        .where('status', isEqualTo: "Status.pending")
        .where('ride.fromText', isEqualTo: ride.ride.fromText)
        .where('ride.toText', isEqualTo: ride.ride.toText);
  ...
  Future<void> deleteRideModelFromUserRide(UserRide userRide) async {
    ...
    var query = ridesCollection
        .where('driver.uid', isEqualTo: userRide.uid)
  ...
  Future<void> deleteUserRide(UserRide userRide) async {
    ...
    var query = userRidesCollection
        .where('uid', isEqualTo: userRide.uid)
